...
export type SectionType = 0 | 1; // 0 = reserved/numbered, 1 = general admission
...
export const GA_KEYWORDS: string[] = [
  "GA",
  "GENERAL ADMISSION",
  "GENERAL",
  "STANDING",
...
export const SECNAM_RULES: Array<{ keywords: string[]; secnam: string }> = [
...
export function deriveType(
  ...
  for (const kw of GA_KEYWORDS) {
    if (upper.includes(kw)) return [1, 1.0];
  }

  // Section ID pattern: /^GA\d*$/i
  if (/^GA\d*$/i.test(sectionId)) return [1, 0.7];

  // Color-named sections are reserved seating with lettered rows
  if (COLOR_SECTION_IDS.has(sectionId.toUpperCase())) return [0, 0.8];

  // Structural inference: no numeric seats → likely GA
  if (!hasNumericSeats) return [1, 0.5];
...
export function deriveSecnam(
  ...
  if (sectionType === 1) return ["GA", 1.0];

  for (const rule of SECNAM_RULES) {
    for (const kw of rule.keywords) {
      if (upper.includes(kw)) return [rule.secnam, 1.0];
    }
  }

  // Purely numeric section ID  e.g. "101", "202"
  if (/^\d+$/.test(sectionId)) return ["SEC", 0.6];

  // Letter-prefixed ID  e.g. "P3" → "P", "AA2" → "AA"
  const prefixMatch = sectionId.match(/^([A-Za-z]+)/);
  if (prefixMatch) return [prefixMatch[1].toUpperCase(), 0.5];
