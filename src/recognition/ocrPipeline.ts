...
async function cropRegion(
  ...
  const rawCrop = await sharp(originalBuffer)
    .extract({
  ...
  if (cropHeight < MIN_HEIGHT) {
    ...
    pipeline = pipeline.resize(
      Math.round(cropWidth * scaleFactor),
      MIN_HEIGHT,
      { fit: "fill" }
    );
  }
