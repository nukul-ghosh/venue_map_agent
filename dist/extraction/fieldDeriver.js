...
const SECTION_ID_RE_COLOR = /\b(PINK|RED|GREEN|YELLOW|CYAN|BLUE|ORANGE|PURPLE|GOLD|SILVER|WHITE|BLACK)\b/gi;
 ...
 */
function extractSectionId(combinedText, clusterId) {
    // Tier 1: alphanumeric combos like "SEC101", "2A" (highest specificity)
    const alphanumMatches = [...combinedText.matchAll(SECTION_ID_RE_ALPHANUM)].map((m) => m[0].toUpperCase());
    if (alphanumMatches.length > 0)
        return [alphanumMatches[0], 1.0];
    // Tier 2: color names used as section identifiers on colored venue maps
    const colorMatches = [...combinedText.matchAll(SECTION_ID_RE_COLOR)].map((m) => m[0].toUpperCase());
    if (colorMatches.length > 0)
        return [colorMatches[0], 0.85];
    ...
    if (alphaMatches.length > 0)
        return [alphaMatches[0], 0.65];
    // Tier 4: standalone numbers like "101", "5"
    const numMatches = [...combinedText.matchAll(SECTION_ID_RE_NUM)].map((m) => m[0]);
    if (numMatches.length > 0)
        return [numMatches[0], 0.5];
 ...
 */
function extractRows(combinedText) {
    const matches = uniqueOrdered([...combinedText.matchAll(ROW_RE)]
    ...
    const singleLetters = matches
        .filter((m) => /^[A-Z]$/.test(m))
        .sort();
    const isConsecutive = singleLetters.length >= 2 &&
        singleLetters.every((letter, i) => i === 0 ||
 ...
 */
function extractSeats(combinedText) {
    const matches = uniqueOrdered([...combinedText.matchAll(SEAT_RE)].map((m) => m[0].toUpperCase()));
 ...
 */
function rollupConfidence(detectionMean, recognitionMean, fieldConf) {
    return (0.3 * detectionMean +
        0.3 * recognitionMean +
 ...
 */
function deriveFields(candidate, learnedOverrides = []) {
    const warnings = [];
    const { combinedText, regions, clusterId } = candidate;
    // Per-region scores
    const detectionMean = regions.reduce((s, r) => s + r.detectionScore, 0) / regions.length;
    const recognitionMean = regions.reduce((s, r) => s + r.recognitionConfidence, 0) / regions.length;
    ...
    if (rows.length === 0)
        warnings.push("no row labels detected");
    const [seats, seatsConf] = extractSeats(combinedText);
    if (seats.length === 0)
        warnings.push("no seat numbers detected");
    const capacity = rows.length > 0 && seats.length > 0
        ? rows.length * seats.length
        : seats.length;
    const capacityConf = capacity > 0 ? 0.8 : 0.0;
    if (capacity === 0)
        warnings.push("capacity defaulted to 0");
    const hasNumericSeats = seats.some((s) => /^\d+$/.test(s));
    const [sectionType, typeConf] = (0, heuristics_1.deriveType)(combinedText, sectionId, hasNumericSeats, learnedOverrides);
    const [secnam, secnamConf] = (0, heuristics_1.deriveSecnam)(combinedText, sectionId, sectionType, learnedOverrides);
    const fieldConf = {
...
//# sourceMappingURL=fieldDeriver.js.map