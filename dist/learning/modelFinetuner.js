...
exports.ModelFinetuner = void 0;
const tf = __importStar(require("@tensorflow/tfjs-node"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)("ModelFinetuner");
/**
...
class ModelFinetuner {
    ...
    modelDir;
    minSamples;
    constructor(dataStore, modelDir, minSamples = 50) {
    ...
    async run() {
        ...
        if (correctionCount < this.minSamples) {
            logger.warn(`Need at least ${this.minSamples} corrections to fine-tune ` +
                `(currently ${correctionCount}). Skipping model fine-tuning.`);
            logger.info("Heuristic overrides are always applied regardless of correction count.");
            return;
        }
        const layersModelPath = path.join(this.modelDir, "layers_model.json");
        if (!fs.existsSync(layersModelPath)) {
            logger.warn(`No LayersModel found at "${layersModelPath}". ` +
...
exports.ModelFinetuner = ModelFinetuner;
//# sourceMappingURL=modelFinetuner.js.map